...
if [ ! -d "${ROOTFS_DIR}" ]; then
    ...
    if [ -n "${PREV_ROOTFS_DIR}" ] && [ -d "${PREV_ROOTFS_DIR}" ]; then
        ...
        STAGE2_ROOTFS=$(find "${BASE_DIR}/work" -path "*/stage2/rootfs" -type d 2>/dev/null | head -1)
        if [ -n "$STAGE2_ROOTFS" ] && [ -d "$STAGE2_ROOTFS" ]; then
            export PREV_ROOTFS_DIR="$STAGE2_ROOTFS"
            echo "Found stage2 rootfs at: $STAGE2_ROOTFS"
        else
